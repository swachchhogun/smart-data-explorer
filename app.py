...
tab_dash, tab_explore, tab_quality, tab_types, tab_stats, tab_ai, tab_report = st.tabs([
    "◈  Dashboard", "◈  Explore", "◈  Data Quality", "◈  Column Types",
...
with tab_dash:
    ...
    st.markdown(
        ...
        f'margin-bottom:1.25rem;line-height:1.7;">DataLens reads your columns and auto-builds the most '
        f'relevant dashboard for your data. Download it as a PNG image or PDF.</div>',
        unsafe_allow_html=True,
    )

    # ── Detect best columns ────────────────────────────────
    num_cols_d  = filtered_df.select_dtypes(include="number").columns.tolist()
    cat_cols_d  = filtered_df.select_dtypes(include="object").columns.tolist()
    date_cols_d = [c for c in filtered_df.columns
    ...
    best_date = date_cols_d[0] if date_cols_d else None
    value_col = num_cols_d[0]  if num_cols_d  else None

    # Try to find a more meaningful "value" column (revenue/sales/profit/score)
    ...
    # ── KPI row ────────────────────────────────────────────
    st.markdown(f'<div style="font-family:DM Mono,monospace;font-size:8px;letter-spacing:3px;'
                f'text-transform:uppercase;color:{T["accent"]};margin-bottom:0.6rem;">Key Metrics</div>',
                unsafe_allow_html=True)
    ...
    if value_col:
        kpis.append({"label": f"Total {value_col}", "value": f"{filtered_df[value_col].sum():,.0f}",
                     "sub": f"avg {filtered_df[value_col].mean():,.1f}"})
        kpis.append({"label": f"Max {value_col}",   "value": f"{filtered_df[value_col].max():,.0f}",
                     "sub": f"min {filtered_df[value_col].min():,.0f}"})
    else:
        kpis.append({"label": "Total Rows", "value": f"{len(filtered_df):,}", "sub": "records"})

    if best_cat:
        top_cat = filtered_df[best_cat].value_counts().index[0]
        top_pct = round(filtered_df[best_cat].value_counts().iloc[0] / len(filtered_df) * 100, 1)
        kpis.append({"label": f"Top {best_cat}", "value": str(top_cat), "sub": f"{top_pct}% of data"})

    kpis.append({"label": "Completeness", "value": f"{completeness}%",
                 "sub": f"{missing_count:,} nulls"})
    kpis.append({"label": "Total Rows",   "value": f"{len(filtered_df):,}",
                 "sub": f"{len(filtered_df.columns)} columns"})
    ...
    for i, kpi in enumerate(kpis[:4]):
        with kpi_cols[i]:
            st.markdown(f"""<div style="background:{T['card']};border:1px solid {T['divider']};
                ...
                <div style="font-family:'DM Mono',monospace;font-size:8px;letter-spacing:2px;
                    text-transform:uppercase;color:{T['text_dim']};margin-bottom:0.3rem;">{kpi['label']}</div>
                <div style="font-family:'Syne',sans-serif;font-size:clamp(18px,3vw,28px);font-weight:800;
                    color:{T['text_head']};line-height:1;letter-spacing:-1px;">{kpi['value']}</div>
                <div style="font-family:'DM Mono',monospace;font-size:9px;color:{T['accent']};
                    margin-top:4px;">{kpi['sub']}</div>
            </div>""", unsafe_allow_html=True)

    # ── Build the dashboard figure ─────────────────────────
    has_trend = best_date and best_num
    has_cat   = best_cat  and value_col
    has_dist  = len(num_cols_d) >= 1
    has_corr  = len(num_cols_d) >= 3

    # Decide layout based on available data
    if has_trend and has_cat:
        rows, specs = 2, [[{"type":"xy"},{"type":"xy"}],[{"type":"xy"},{"type":"domain"}]]
        subplot_titles = [
    ...
    elif has_cat and has_dist:
        rows, specs = 2, [[{"type":"xy"},{"type":"domain"}],[{"type":"xy"},{"type":"xy"}]]
        subplot_titles = [
            f"{value_col or best_num} by {best_cat}",
            f"Share by {best_cat}",
            f"Distribution — {best_num}",
            f"Top 10 by {best_cat}",
        ]
    else:
        rows, specs = 2, [[{"type":"xy"},{"type":"xy"}],[{"type":"xy"},{"type":"xy"}]]
        subplot_titles = ["Distribution","Spread","Trend","Summary"]
    ...
    DASH_COLORS = COLORS if 'COLORS' in dir() else ["#ff5a32","#50b4ff","#a8e063","#f7c948","#c678dd","#56b6c2"]

    try:
        if has_trend and has_cat:
            # Panel 1: Trend line
            trend_df = filtered_df[[best_date, value_col]].dropna().sort_values(best_date)
            fig_dash.add_trace(go.Scatter(
                x=trend_df[best_date], y=trend_df[value_col],
                mode="lines", name=value_col,
                line=dict(color=T["accent"], width=2),
                fill="tozeroy", fillcolor=T["accent_bg"],
            ), row=1, col=1)

            # Panel 2: Bar by category
            cat_grp = filtered_df.groupby(best_cat)[value_col].sum().nlargest(10).reset_index()
            for idx, (_, row_data) in enumerate(cat_grp.iterrows()):
                fig_dash.add_trace(go.Bar(
                    x=[row_data[best_cat]], y=[row_data[value_col]],
                    name=str(row_data[best_cat]),
                    marker_color=DASH_COLORS[idx % len(DASH_COLORS)],
                    showlegend=False,
                ), row=1, col=2)

            # Panel 3: Histogram
            fig_dash.add_trace(go.Histogram(
                x=filtered_df[best_num].dropna(),
                nbinsx=25,
                marker_color=T["blue"],
                opacity=0.8,
                name=best_num,
            ...
            cat_grp = filtered_df.groupby(best_cat)[value_col or best_num].sum().nlargest(10).reset_index()
            vcol = value_col or best_num
            fig_dash.add_trace(go.Bar(
                y=cat_grp[best_cat].tolist(),
                x=cat_grp[vcol].tolist(),
                orientation="h",
            ...
            # Panel 2: Pie
            pie_df = filtered_df[best_cat].value_counts().head(7)
            fig_dash.add_trace(go.Pie(
                labels=pie_df.index.tolist(),
                values=pie_df.values.tolist(),
                hole=0.4,
            ...
            # Panel 4: Box plots for all numeric cols (up to 6)
            for idx, nc in enumerate(num_cols_d[:6]):
                fig_dash.add_trace(go.Box(
                    y=filtered_df[nc].dropna(),
                    name=nc,
                    marker_color=DASH_COLORS[idx % len(DASH_COLORS)],
                    boxmean=True,
                ), row=2, col=2)
            ...
            # Fallback: pure numeric layout
            for idx, nc in enumerate(num_cols_d[:3]):
                fig_dash.add_trace(go.Histogram(
                    x=filtered_df[nc].dropna(), nbinsx=20,
                    name=nc, marker_color=DASH_COLORS[idx % len(DASH_COLORS)],
                    opacity=0.75,
                ), row=1, col=1)

            if len(num_cols_d) >= 2:
                fig_dash.add_trace(go.Scatter(
                    x=filtered_df[num_cols_d[0]].dropna(),
                    y=filtered_df[num_cols_d[1]].dropna(),
                    mode="markers",
            ...
            for idx, nc in enumerate(num_cols_d[:6]):
                fig_dash.add_trace(go.Box(
                    y=filtered_df[nc].dropna(), name=nc,
                    marker_color=DASH_COLORS[idx % len(DASH_COLORS)],
                ), row=2, col=1)

            if len(num_cols_d) >= 2:
                corr_m = filtered_df[num_cols_d[:6]].corr()
        ...
        st.warning(f"Dashboard build error: {e}")

    # ── Style the dashboard figure ─────────────────────────
    fig_dash.update_layout(
        ...
        font=dict(family="DM Sans", color=T["text"], size=11),
        title=dict(
            text=f"<b>DataLens Dashboard</b>  ·  {fn}  ·  {len(filtered_df):,} rows",
    ...
    fig_dash.update_xaxes(**axis_style)
    fig_dash.update_yaxes(**axis_style)
    ...
    st.plotly_chart(fig_dash, use_container_width=True, config={
        "displayModeBar": True,
        "modeBarButtonsToAdd": ["drawrect", "drawopenpath"],
    ...
    with dl1:
        # CSV of the filtered data powering this dashboard
        st.download_button(
            "↓ Download data (CSV)",
            data=filtered_df.to_csv(index=False).encode("utf-8"),
            file_name=f"datalens_{fn.replace('.csv','')}_filtered.csv",
            mime="text/csv",
    ...
    with dl3:
        st.markdown(
            f'<div style="font-family:DM Mono,monospace;font-size:8px;letter-spacing:1px;'
            f'text-transform:uppercase;color:{T["text_faint"]};padding:0.7rem 0;text-align:center;">'
            f'Use the 📷 camera icon on the chart to save as PNG</div>',
            unsafe_allow_html=True,
        )

    _date_txt = "trend detected" if best_date else "no date column"
    st.markdown(
        f'<div style="font-family:DM Mono,monospace;font-size:8px;letter-spacing:1px;'
        f'text-transform:uppercase;color:{T["text_faint"]};margin-top:0.5rem;">'
        f'◈ Dashboard auto-built from {len(filtered_df.columns)} columns · '
        f'{_date_txt} · '
        f'{len(num_cols_d)} numeric · {len(cat_cols_d)} categorical</div>',
        unsafe_allow_html=True,
    )
